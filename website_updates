...
# === Streamlit App ===
st.title("📄 WIAT-4 Report Filler")

uploaded_doc = st.file_uploader("Upload WIAT-4 Report (.docx)", type="docx")

TEMPLATE_PATH = Path("template.docx")  # Your template saved inside project

if uploaded_doc:
    if st.button("Generate Filled Template"):
        input_doc = Document(uploaded_doc)
        template_doc = Document(TEMPLATE_PATH)  # Load fresh template each time
